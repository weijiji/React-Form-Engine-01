...
### 审批链引用（Approval chain reference）

模板审批链对审批人的指向方式，三种类型决定治理策略：**直接引用**（`specific`，指名某用户）、**角色引用**（`role`，按角色在运行时解析成员）、**组织引用**（`org_structure`，按提交人的直属上级/部门在运行时解析，不指名用户）。直接引用需变更才可解；角色引用自动路由给存活成员（无需改链）；组织引用不指名用户，改链无效（ADR-0015）。

_Avoid_: 审批人配置、审批链配置

### 停用（Deactivation）

用户被停用后的状态。**被停用用户不得被解析为可用的审批人**——提交解析到被停用审批人时被干净拦截。停用与删除（硬删）是两种操作：**停用**允许带引用进行（离职流程：提醒 + 可跳转，不软拦截）；**删除**要求先解除审批链引用（被引用则拒绝）。
