...
**门户角色**是系统固定的五种角色，决定用户登录后进入哪个门户：

枚举：`模板设计者`、`表单填写者`、`审批人`、`系统管理员`、`运维人员`。

**权限角色**（`roles` 表）是权限码（permission code）的集合，决定用户拥有的具体能力；由系统管理员在后台动态创建与维护（工单 09），seed 预置「管理员」「设计者」「填写者」「审批者」「运维」五个角色（工单 17 由原「管理员」「普通用户」拆分而来）。
